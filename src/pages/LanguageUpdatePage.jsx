...
export default function LanguageUpdatePage() {
  ...
  const [preferences, setPreferences] = useState({ announcements: true, voicePrompts: true, printLabels: false });

  const activeLanguage = useMemo(() => languages.find((language) => language.code === selectedLanguage), [selectedLanguage]);
      ...
      <div className="page-stack">
        <section className="hero-panel">
          ...
          <aside className="hero-side">
            ...
            <div className="mini-stat-grid">
              <div className="mini-stat"><p className="metric-label">Coverage</p><strong>{activeLanguage?.adoption}%</strong></div>
              <div className="mini-stat"><p className="metric-label">Active merchants</p><strong>1,248</strong></div>
              <div className="mini-stat"><p className="metric-label">Device sync</p><strong>24 / 26</strong></div>
              <div className="mini-stat"><p className="metric-label">Pending approvals</p><strong>02</strong></div>
            </div>
          </aside>
        </section>
        ...
        <section className="language-layout">
          <div className="language-card">
            <div className="section-header">
              <div>
                ...
                <p className="section-copy">Each option includes readiness across interface labels, alerts and device voice confirmations.</p>
              </div>
            </div>
            <div className="language-list">
              {languages.map((language) => (
                <article key={language.code} className={`language-option ${selectedLanguage === language.code ? 'selected' : ''}`} onClick={() => setSelectedLanguage(language.code)}>
                  <div className="language-option-top">
                    ...
                    {selectedLanguage === language.code ? <span className="selection-indicator"><Check size={14} />Selected</span> : null}
                  </div>
          ...
          <aside className="promo-card">
            ...
            <p className="section-copy">{activeLanguage?.nativeName} will be used for the merchant portal interface, quick actions, field device prompts and QR standee text where supported.</p>
            <div className="insight-list">
              <div className="insight-item"><span>Primary region</span><strong>{activeLanguage?.region}</strong></div>
              <div className="insight-item"><span>Audio prompt readiness</span><strong>Voice confirmations available</strong></div>
              <div className="insight-item"><span>Fallback language</span><strong>English retained for compliance labels</strong></div>
            </div>
          </aside>
        </section>
        ...
        <section className="content-grid">
              ...
              {preferenceItems.map((item) => (
                <div key={item.key} className="toggle-row">
                  ...
                  <label className="toggle-switch">
                    <input type="checkbox" checked={preferences[item.key]} onChange={() => setPreferences((current) => ({ ...current, [item.key]: !current[item.key] }))} />
          ...
          <aside className="summary-card">
            ...
            <div className="insight-list">
              <div className="insight-item"><span>Portal UI</span><strong>Menus, labels, alerts and reconciliation prompts</strong></div>
              <div className="insight-item"><span>Merchant devices</span><strong>Audio confirmations and in-device status messages</strong></div>
              <div className="insight-item"><span>Printed output</span><strong>Receipt captions and QR standee download text</strong></div>
            </div>
            <button type="button" className="ghost-button"><MonitorSpeaker size={18} />Review device sync</button>
            <button type="button" className="ghost-button"><Volume2 size={18} />Test voice prompts</button>
            <button type="button" className="ghost-button"><Mic2 size={18} />Review audio fallback</button>
          </aside>
        </section>
      </div>
    </Layout>
